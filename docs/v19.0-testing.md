...
| Drupal security alerts use current tooling | Run the scheduled helper as its configured cron identity, verify the wrapper's effective user, ask Drush for help on `pm:security` and audit the lock file | Root starts the helper and `turnkey-drush` drops to the web user; the current command exists and the installed dependency set has no known advisory | `tests/v19.sh` |
